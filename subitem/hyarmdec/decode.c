 ...
 * @Date: 2023-10-10 17:13:11
 * @LastEditors: lancerstadium lancerstadium@163.com
 * @LastEditTime: 2023-10-11 12:54:39
 * @FilePath: /hybitor_effect/subitem/hyarmdec/decode.c
 * @Description: ARM64 指令解码器实现
 */
...
#define HYARMDEC_INTERNAL 1 // 定义"hyarmdec.h"为在该文件中外部使用

#include "hyarmdec.h"
#include "hyarminsts.h"
...
static Inst errinst(char *err) {
	Inst inst = UNKNOWN_INST;   // 未知指令
	inst.op = ARM64_ERROR,      // 指令
	inst.error = err;           // 错误信息
	return inst;
}
...
static u8 invert_cond(u8 flags) {
	Cond cond = arm64_get_cond(flags);
...
/// @param va 向量数据大小
/// @return 浮点数据大小
FPSize arm64_size_from_vec_arrangement(VectorArrangement va) {
	return (FPSize)(va >> 1);
}


/// @brief 如果存在目的地寄存器 Rd，则占用0..4位；且31号寄存器被视为零/丢弃寄存器ZR/WZR。
/// @param binst 指令
/// @return 寄存器
static inline Reg regRd(u32 binst) {
	return binst & 0b11111;
}

/// @brief 31号寄存器被视为堆栈指针 SP。
/// @param binst 二进制指令
/// @return 寄存器
static inline Reg regRdSP(u32 binst) {
	Reg rd = binst & 0b11111;
	return (rd == 31) ? STACK_POINTER : rd;
}

/// @brief 第一个操作数寄存器Rn，如果存在，则占用5..9位；且31号寄存器被视为零/丢弃寄存器ZR/WZR。
/// @param binst 二进制指令
/// @return 寄存器
...
/// @brief 第二个操作数寄存器Rm，如果存在，则占用位16..20。31号寄存器被视为零/丢弃寄存器ZR/WZR。
/// @param binst 二进制指令
/// @return 寄存器
static inline Reg regRm(u32 binst) {
...
/// @param binst 二进制指令
/// @return 寄存器
static inline Reg regRmSP(u32 binst) {
	Reg rm = (binst >> 16) & 0b11111;
	return (rm == 31) ? STACK_POINTER : rm;
}


/// @brief Sext符号-将b位数字以x扩展为64位。上（64-b）位必须为零。很少需要，但很麻烦。
/// @param x 拓展数字
/// @param b 数字
/// @return 拓展数字
...
/// @brief Helpers for data_proc_imm
/// @param opcode 
/// @param w32 
/// @param rd 
/// @param rn 
...
/// @param imms 
/// @return 
Inst find_bfm_alias(Opcode opcode, bool w32, Reg rd, Reg rn, u8 immr, u8 imms);

u64 decode_bitmask(u8 immN, u8 imms, u8 immr, bool w32);
...
static Inst data_proc_imm(u32 binst) {
	...
	case PCRelAddr: {
		...
		u64 scale = (inst.op == ARM64_ADRP) ? 4096 : 1; // ADRP: Page (4K) Address
	...
	case AddSub: {
		...
		inst.op = (is_add) ? ARM64_ADD_IMM : ARM64_SUB_IMM;
		if (top3 & 0b001)
		...
		if (inst.rd == ZERO_REG && (inst.flags & SET_FLAGS)) {
			switch (inst.op) {
			case ARM64_ADD_IMM: inst.op = ARM64_CMN_IMM; break;
			case ARM64_SUB_IMM: inst.op = ARM64_CMP_IMM; break;
			default:
				break; // impossible; shuts up warning
			}
		} else if (inst.op == ARM64_ADD_IMM && !shift_by_12 && unshifted_imm == 0 &&
			((inst.rd == STACK_POINTER) || (inst.rn == STACK_POINTER))) {
			inst.op = ARM64_MOV_SP;
		}
		break;
	}
	case Logic: {
		switch (top3 & 0b011) {
		case 0b00: inst.op = ARM64_AND_IMM; break;
		case 0b01: inst.op = ARM64_ORR_IMM; break;
		case 0b10: inst.op = ARM64_EOR_IMM; break;
		case 0b11:
			inst.op = (regRd(binst) == ZERO_REG) ? ARM64_TST_IMM : ARM64_AND_IMM;
			inst.flags |= SET_FLAGS;
			break;
		}
	...
	case Bitfield: {
		Opcode op = ARM64_UNKNOWN;

		switch (top3 & 0b011) { // base instructions
		case 0b00: op = ARM64_SBFM; break;
		case 0b01: op = ARM64_BFM;  break;
		case 0b10: op = ARM64_UBFM; break;
		default:
...
Inst find_bfm_alias(Opcode op, bool w32, Reg rd, Reg rn, u8 immr, u8 imms) {
	Inst inst = UNKNOWN_INST;
	...
	if (op == ARM64_BFM) {
		...
		inst.op = (rn == ZERO_REG) ? ARM64_BFC : ARM64_BFI;
	...
	if (imms == all_ones) {
		inst.op = (sign) ? ARM64_ASR_IMM : ARM64_LSR_IMM;
		inst.imm = immr;
		return inst;
	}
	...
	if (imms < immr) {
		inst.op = (sign) ? ARM64_SBFIZ : ARM64_UBFIZ;
		inst.bfm.lsb = bits - immr;
		inst.bfm.width = imms + 1;
		return inst;
	}
	...
	if (immr == 0) {
		switch (imms) {
		case  7: inst.op = ARM64_EXTEND; inst.extend.type = (sign) ? SXTB : UXTB; return inst;
		case 15: inst.op = ARM64_EXTEND; inst.extend.type = (sign) ? SXTH : UXTH; return inst;
		case 31:
			inst.op = ARM64_EXTEND;
	...
	inst.op = (sign) ? ARM64_SBFX : ARM64_UBFX;
	inst.bfm.lsb = immr;
	inst.bfm.width = imms - immr + 1;
	return inst;
}
...
static Inst branches(u32 binst) {
	...
	case System: {
		...
		if (exc) {
			...
			switch (opcLL) {
			case 0b00001: inst.op = ARM64_SVC; break; // system call
			case 0b00010: inst.op = ARM64_HVC; break;
			case 0b00011: inst.op = ARM64_SMC; break;
			case 0b00100: inst.op = ARM64_BRK; break;
			case 0b01000: inst.op = ARM64_HLT; break;
			case 0b10101: inst.op = ARM64_DCPS1; break;
			case 0b10110: inst.op = ARM64_DCPS2; break;
			case 0b10111: inst.op = ARM64_DCPS3; break;
			default:
		...
		if (msr) {
			...
			inst.op = (load) ? ARM64_MRS : ARM64_MSR_REG;
		...
		if (sys) {
			bool load = (binst >> 21) & 1;
			inst.op = (load) ? ARM64_SYSL : ARM64_SYS;
		...
		if (pstate) {
			...
			case 0b000000: inst.op = ARM64_CFINV;  return inst;
			case 0b000001: inst.op = ARM64_XAFlag; return inst;
			case 0b000010: inst.op = ARM64_AXFlag; return inst;
		...
		switch (op2) {
		case 0b010: inst.op = ARM64_CLREX; inst.imm = crm; break;
		case 0b101: inst.op = ARM64_DMB; inst.imm = crm; break;
		case 0b110: inst.op = ARM64_ISB; inst.imm = crm; break;
		case 0b111:
		...
		case 0b100:
			switch (crm) {
			case 0b0000: inst.op = ARM64_SSBB; break;
			case 0b0100: inst.op = ARM64_PSSBB; break;
			default:
				inst.op = ARM64_DSB;
				inst.imm = crm;
				break;
	...
	case UncondBranchReg: {
		...
		switch (op) {
		case 0b00: inst.op = ARM64_BR;  break;
		case 0b01: inst.op = ARM64_BLR; break;
		case 0b10: inst.op = ARM64_RET; break;
		default:
			return UNKNOWN_INST;
		}
	...
	case UncondBranch:
		inst.op = (binst & (1<<31)) ? ARM64_BL : ARM64_B; // MSB = 1 → BL
	...
	case CmpAndBranch: {
		...
		inst.op = (zero) ? ARM64_CBZ : ARM64_CBNZ;
	...
	case TestAndBranch: {
		...
		bool zero = (binst & (1 << 24)) == 0;
		inst.op = (zero) ? ARM64_TBZ : ARM64_TBNZ;
...
static Inst data_proc_reg(u32 binst) {
	...
	case DataProc2: {
		...
		case 0b000000:
			...
			return inst;
		case 0b000010: inst.op = ARM64_UDIV; break;
		case 0b000011: inst.op = ARM64_SDIV; break;
		case 0b001000: inst.op = ARM64_LSLV; break; // lowest two bits → enum Shift
		case 0b001001: inst.op = ARM64_LSRV; break; // --
		case 0b001010: inst.op = ARM64_ASRV; break; // --
		case 0b001011: inst.op = ARM64_RORV; break; // --
		case 0b010000: inst.op = ARM64_CRC32B; break; // lowest two bits → enum Size
		case 0b010001: inst.op = ARM64_CRC32H; break;
		case 0b010010: inst.op = ARM64_CRC32W; break;
		case 0b010011: inst.op = ARM64_CRC32X; break;
		case 0b010100: inst.op = ARM64_CRC32CB; break;
		case 0b010101: inst.op = ARM64_CRC32CH; break;
		case 0b010110: inst.op = ARM64_CRC32CW; break;
		case 0b010111: inst.op = ARM64_CRC32CX; break;
		}
	...
	case DataProc1:
		...
		case 0b000: inst.op = ARM64_RBIT; break;
		case 0b001: inst.op = ARM64_REV16; break;
		case 0b010: inst.op = (inst.flags & W32) ? ARM64_REV : ARM64_REV32; break;
		case 0b011: inst.op = ARM64_REV; break;
		case 0b100: inst.op = ARM64_CLZ; break;
		case 0b101: inst.op = ARM64_CLS; break;
		}
	...
	case LogicalShifted: {
		...
		switch (top3 & 0b011) {
		case 0b00: inst.op = (negate) ? ARM64_BIC : ARM64_AND_SHIFTED; break;
		case 0b01: inst.op = (negate) ? ARM64_ORN : ARM64_ORR_SHIFTED; break;
		case 0b10: inst.op = (negate) ? ARM64_EON : ARM64_EOR_SHIFTED; break;
		case 0b11:
			inst.op = (negate) ? ARM64_BIC : ARM64_AND_SHIFTED;
		...
		if (inst.op == ARM64_AND_SHIFTED && (inst.flags & SET_FLAGS) && inst.rd == ZERO_REG)
			inst.op = ARM64_TST_SHIFTED;
		else if (inst.op == ARM64_ORR_SHIFTED && shift == 0 && imm6 == 0 && inst.rn == ZERO_REG)
			inst.op = ARM64_MOV_REG;
		else if (inst.op == ARM64_ORN && inst.rn == ZERO_REG)
			inst.op = ARM64_MVN;
		break;
	}
	case AddSubShifted:
		...
		case 0b00:
		case 0b01:
			inst.op = ARM64_ADD_SHIFTED;
			break;
		case 0b10:
		case 0b11:
			inst.op = ARM64_SUB_SHIFTED;
			break;
		}
		...
		if (inst.rd == ZERO_REG && (inst.flags & SET_FLAGS)) {
			switch (inst.op) {
			case ARM64_ADD_SHIFTED: inst.op = ARM64_CMN_SHIFTED; break;
			case ARM64_SUB_SHIFTED: inst.op = ARM64_CMP_SHIFTED; break;
			default:
				break; // impossible; shuts up warning
			}
		} else if (inst.op == ARM64_SUB_SHIFTED && inst.rn == ZERO_REG) {
			inst.op = ARM64_NEG;
		}
		break;
	...
	case AddSubExtended: {
		...
		case 0b00:
		case 0b01:
			inst.op = ARM64_ADD_EXT;
			break;
		case 0b10:
		case 0b11:
			inst.op = ARM64_SUB_EXT;
			break;
		}
		...
		if (inst.rd == ZERO_REG && (inst.flags & SET_FLAGS)) {
			switch (inst.op) {
			case ARM64_ADD_EXT: inst.op = ARM64_CMN_EXT; break;
			case ARM64_SUB_EXT: inst.op = ARM64_CMP_EXT; break;
			default:
				break; // impossible; shuts up warning
			}
		}
	...
	case AddSubCarryAndFlagManipulation: {
		...
		case 0b000000: {
			bool sub = top3 & 0b010;
			inst.op = (sub) ? ARM64_SBC : ARM64_ADC;
			inst.rd = regRd(binst);
			inst.rn = regRn(binst);
			inst.rm = regRm(binst);
			if (inst.op == ARM64_SBC && inst.rn == ZERO_REG) {
				inst.op = ARM64_NGC;
			}
			break;
		}
	...
	case CondCompare: {
		...
		if (positive) {
			inst.op = (immediate) ? ARM64_CCMN_IMM : ARM64_CCMN_REG;
		} else {
			inst.op = (immediate) ? ARM64_CCMP_IMM : ARM64_CCMP_REG;
		}
	...
	case CondSelect: {
		...
		switch (op) {
		case 0b000: inst.op = ARM64_CSEL; break;
		case 0b001: inst.op = ARM64_CSINC; break;
		case 0b100: inst.op = ARM64_CSINV; break;
		case 0b101: inst.op = ARM64_CSNEG; break;
		default:
			return UNKNOWN_INST;
		}
		...
		if (inst.rm == ZERO_REG && inst.rn == ZERO_REG) {
			switch (inst.op) {
			case ARM64_CSINC: inst.op = ARM64_CSET; inst.flags = invert_cond(inst.flags); break;
			case ARM64_CSINV: inst.op = ARM64_CSETM; inst.flags = invert_cond(inst.flags); break;
			default:
				break;
			}
		} else if (inst.rm == inst.rn && inst.op == ARM64_CSNEG) {
			inst.op = ARM64_CNEG;
			inst.flags = invert_cond(inst.flags);
		} else if (inst.rm == inst.rn && inst.op == ARM64_CSINC) {
			inst.op = ARM64_CINC;
			inst.flags = invert_cond(inst.flags);
		} else if (inst.rm == inst.rn && inst.op == ARM64_CSINV) {
			inst.op = ARM64_CINV;
			inst.flags = invert_cond(inst.flags);
		}
		break;
	...
	case DataProc3: {
		...
		case 0b000: inst.op = (sub) ? ARM64_MSUB : ARM64_MADD; break;
		case 0b001: inst.op = (sub) ? ARM64_SMSUBL : ARM64_SMADDL; break;
		case 0b010: inst.op = ARM64_SMULH; break;
		case 0b101: inst.op = (sub) ? ARM64_UMSUBL : ARM64_UMADDL; break;
		case 0b110: inst.op = ARM64_UMULH; break;
		default:
			return errinst("data_proc_reg/DataProc3: invalid opcode");
		}

		// Only MADD und MSUB have 32-bit variants.
		if ((top3 & 0b100) == 0 && inst.op != ARM64_MADD && inst.op != ARM64_MSUB)
		...
		if (inst.ra == ZERO_REG) {
			switch (inst.op) {
			case ARM64_MADD:   inst.op =    ARM64_MUL; break;
			case ARM64_MSUB:   inst.op =   ARM64_MNEG; break;
			case ARM64_SMADDL: inst.op =  ARM64_SMULL; break;
			case ARM64_SMSUBL: inst.op = ARM64_SMNEGL; break;
			case ARM64_UMADDL: inst.op =  ARM64_UMULL; break;
			case ARM64_UMSUBL: inst.op = ARM64_UMNEGL; break;
...
static Inst loads_and_stores(u32 binst) {
	...
	case SIMDMult: {
		...
		case 0b0000: inst.op = (load) ? ARM64_LD4_MULT : ARM64_ST4_MULT; nreg = 4; break;
		case 0b0010: inst.op = (load) ? ARM64_LD1_MULT : ARM64_ST1_MULT; nreg = 4; break;
		case 0b0100: inst.op = (load) ? ARM64_LD3_MULT : ARM64_ST3_MULT; nreg = 3; break;
		case 0b0110: inst.op = (load) ? ARM64_LD1_MULT : ARM64_ST1_MULT; nreg = 3; break;
		case 0b0111: inst.op = (load) ? ARM64_LD1_MULT : ARM64_ST1_MULT; nreg = 1; break;
		case 0b1000: inst.op = (load) ? ARM64_LD2_MULT : ARM64_ST2_MULT; nreg = 2; break;
		case 0b1010: inst.op = (load) ? ARM64_LD1_MULT : ARM64_ST1_MULT; nreg = 2; break;
		}
		...
		if (arm64_get_addrmode(inst.flags) == AM_POST) {
			if (inst.rm == ZERO_REG) {
				switch (Q) {
	...
	case SIMDSingle: {
		...
		if (load) {
			...
			case 0b000: inst.op = (even) ? ARM64_LD2_SINGLE : ARM64_LD1_SINGLE; size = FSZ_B; break;
			case 0b001: inst.op = (even) ? ARM64_LD4_SINGLE : ARM64_LD3_SINGLE; size = FSZ_B; break;
			case 0b010: inst.op = (even) ? ARM64_LD2_SINGLE : ARM64_LD1_SINGLE; size = FSZ_H; break;
			case 0b011: inst.op = (even) ? ARM64_LD4_SINGLE : ARM64_LD3_SINGLE; size = FSZ_H; break;
			case 0b100:
				inst.op = (even) ? ARM64_LD2_SINGLE : ARM64_LD1_SINGLE;
				size = (size_field == 0b01) ? FSZ_D : FSZ_S;
				break;
			case 0b101:
				inst.op = (even) ? ARM64_LD4_SINGLE : ARM64_LD3_SINGLE;
				size = (size_field == 0b01) ? FSZ_D : FSZ_S;
				break;
			case 0b110: inst.op = (even) ? ARM64_LD2R       : ARM64_LD1R; size = (binst >> 10) & 0b11; break;
			case 0b111: inst.op = (even) ? ARM64_LD4R       : ARM64_LD3R; size = (binst >> 10) & 0b11; break;
			}
		} else {
			switch ((binst >> 13) & 0b111) { // opcode(3)
			case 0b000: inst.op = (even) ? ARM64_ST2_SINGLE : ARM64_ST1_SINGLE; size = FSZ_B; break;
			case 0b001: inst.op = (even) ? ARM64_ST4_SINGLE : ARM64_ST3_SINGLE; size = FSZ_B; break;
			case 0b010: inst.op = (even) ? ARM64_ST2_SINGLE : ARM64_ST1_SINGLE; size = FSZ_H; break;
			case 0b011: inst.op = (even) ? ARM64_ST4_SINGLE : ARM64_ST3_SINGLE; size = FSZ_H; break;
			case 0b100:
				inst.op = (even) ? ARM64_ST2_SINGLE : ARM64_ST1_SINGLE;
				size = (size_field == 0b01) ? FSZ_D : FSZ_S;
				break;
			case 0b101:
				inst.op = (even) ? ARM64_ST4_SINGLE : ARM64_ST3_SINGLE;
				size = (size_field == 0b01) ? FSZ_D : FSZ_S;
				break;
		...
		switch (inst.op) {
		case ARM64_LD1R: case ARM64_LD1_SINGLE: case ARM64_ST1_SINGLE: nreg = 1; break;
		case ARM64_LD2R: case ARM64_LD2_SINGLE: case ARM64_ST2_SINGLE: nreg = 2; break;
		case ARM64_LD3R: case ARM64_LD3_SINGLE: case ARM64_ST3_SINGLE: nreg = 3; break;
		case ARM64_LD4R: case ARM64_LD4_SINGLE: case ARM64_ST4_SINGLE: nreg = 4; break;
		default: break;
		}
	...
	case Exclusive: {
		...
		if (load) {
			inst.op = (exclusive) ? ARM64_LDXR : ARM64_LDR;
			if (exclusive)
				inst.ldst_order.load = (o0) ? MO_ACQUIRE : MO_NONE;
			else
				inst.ldst_order.load = (o0) ? MO_LO_ACQUIRE : MO_ACQUIRE;
		} else {
			inst.op = (exclusive) ? ARM64_STXR : ARM64_STR;
			if (exclusive)
				inst.ldst_order.store = (o0) ? MO_RELEASE : MO_NONE;
			else
	...
	case Literal: {
		...
		inst.op = (vector) ? ARM64_LDR_FP : ARM64_LDR;
		if (vector) {
			switch (top2) { // opc
			...
			switch (top2) { // opc
			case 0b00: inst.flags = W32 | set_mem_extend(inst.flags, (ExtendType)SZ_W); break; // 32-bit LDR
			case 0b01: inst.flags = set_mem_extend(inst.flags, (ExtendType)SZ_X); break;       // 64-bit LDR
			case 0b10: inst.flags = set_mem_extend(inst.flags, (ExtendType)SXTW); break;       // LDRSW
			case 0b11: inst.op = ARM64_PRFM; break;                                  // PRFM (literal)
			}
		}
	...
	case Pair: {
		...
		if (vector) {
			inst.op = (load) ? ARM64_LDP_FP : ARM64_STP_FP;
			switch (top2) {
			...
			inst.op = (load) ? ARM64_LDP : ARM64_STP;
			switch (top2) {
			case 0b00: scale = 4; inst.flags = W32 | set_mem_extend(inst.flags, (ExtendType)SZ_W); break; // 32-bit
			case 0b01: scale = 4; inst.flags = set_mem_extend(inst.flags, (ExtendType)SXTW); break;       // LDPSW, STPSW
			case 0b10: scale = 8; inst.flags = set_mem_extend(inst.flags, (ExtendType)SZ_X); break;       // 64-bit
			default:
				return errinst("loads_and_stores: invalid opc for LDP/STP");
			}
		}
		...
		if (kind == PairNoAlloc) {
			switch (inst.op) {
			case ARM64_LDP:    inst.op = ARM64_LDNP; break;
			case ARM64_STP:    inst.op = ARM64_STNP; break;
			case ARM64_LDP_FP: inst.op = ARM64_LDNP_FP; break;
			case ARM64_STP_FP: inst.op = ARM64_STNP_FP; break;
			default:
	...
	case Register: {
		...
		if (vector) {
			...
			inst.op = (load) ? ARM64_LDR_FP : ARM64_STR_FP;
			...
			case 0b10: // load, sign-extend to 64 bit
				if (size == SZ_X) { // PRFM (unsigned immediate, extended register or unscaled offset)
					...
					if (arm64_get_addrmode(inst.flags) == AM_OFF_EXT) {
			...
			inst.op = (load) ? ARM64_LDR : ARM64_STR;
			if (w32) {
				inst.flags |= W32;
			}
		...
		AddrMode mode = arm64_get_addrmode(inst.flags);
		switch (mode) {
		default:
			return UNKNOWN_INST;
	...
	case Atomic: {
		...
		switch (o3opc) {
		case 0b0000: inst.op = ARM64_LDADD;  break;
		case 0b0001: inst.op = ARM64_LDCLR;  break;
		case 0b0010: inst.op = ARM64_LDEOR;  break;
		case 0b0011: inst.op = ARM64_LDSET;  break;
		case 0b0100: inst.op = ARM64_LDSMAX; break;
		case 0b0101: inst.op = ARM64_LDSMIN; break;
		case 0b0110: inst.op = ARM64_LDUMAX; break;
		case 0b0111: inst.op = ARM64_LDUMIN; break;
		case 0b1000: inst.op = ARM64_SWP;    break;
		case 0b1100:
...
static Inst scalar_floating_point(u32 binst) {
	...
	case DataProc3: {
		...
		switch ((o1 << 1) | o0) {
		case 0b00: inst.op = ARM64_FMADD;  break;
		case 0b01: inst.op = ARM64_FMSUB;  break;
		case 0b10: inst.op = ARM64_FNMADD; break;
		case 0b11: inst.op = ARM64_FNMSUB; break;
		}
	...
	case ConvFixed: {
		...
		switch (opcode) {
		case 0b000: inst.op = ARM64_FCVT_GPR; inst.fcvt.sgn = true; break;
		case 0b001: inst.op = ARM64_FCVT_GPR; inst.fcvt.sgn = false; break;
		case 0b010: inst.op = ARM64_CVTF; inst.fcvt.sgn = true; break;
		case 0b011: inst.op = ARM64_CVTF; inst.fcvt.sgn = false; break;
		default:
	...
	case ConvInt: {
		...
		case 0b010: inst.op = ARM64_CVTF; inst.fcvt.sgn = true;  break;
		case 0b011: inst.op = ARM64_CVTF; inst.fcvt.sgn = false; break;
		case 0b100: inst.op = ARM64_FCVT_GPR; inst.fcvt.sgn = true;  inst.fcvt.mode = FPR_TIE_AWAY; break; // FCVTAS
		case 0b101: inst.op = ARM64_FCVT_GPR; inst.fcvt.sgn = false; inst.fcvt.mode = FPR_TIE_AWAY; break; // FCVTAU
		case 0b110:
			// FMOV: Clear unused immediate again.
			switch (rmode) {
			case 0b00: inst.imm = 0; inst.op = ARM64_FMOV_VEC2GPR; break;
			case 0b01: inst.imm = 0; inst.op = ARM64_FMOV_TOP2GPR; break;
			case 0b11:
				inst.op = ARM64_FJCVTZS;
				inst.fcvt.mode = FPR_ZERO;
				inst.fcvt.sgn = true;
				break;
			default:
		...
		case 0b111:
			// FMOV: Clear unused immediate again.
			switch (rmode) {
			case 0b00: inst.imm = 0; inst.op = ARM64_FMOV_GPR2VEC; break;
			case 0b01: inst.imm = 0; inst.op = ARM64_FMOV_GPR2TOP; break;
			default:
	...
	case DataProc1: {
		...
		switch (opcode) {
		case 0b000000: inst.op = ARM64_FMOV_REG; break;
		case 0b000001: inst.op = ARM64_FABS; break;
		case 0b000010: inst.op = ARM64_FNEG; break;
		case 0b000011: inst.op = ARM64_FSQRT; break;
		case 0b000100: inst.op = ARM64_FCVT_S; break;
		case 0b000101: inst.op = ARM64_FCVT_D; break;
		case 0b000111: inst.op = ARM64_FCVT_H; break;
		case 0b001000: inst.op = ARM64_FRINT; inst.frint.mode = FPR_TIE_EVEN; break; // frintN
		case 0b001001: inst.op = ARM64_FRINT; inst.frint.mode = FPR_POS_INF;  break; // frintP
		case 0b001010: inst.op = ARM64_FRINT; inst.frint.mode = FPR_NEG_INF;  break; // frintM
		case 0b001011: inst.op = ARM64_FRINT; inst.frint.mode = FPR_ZERO;     break; // frintZ
		case 0b001100: inst.op = ARM64_FRINT; inst.frint.mode = FPR_TIE_AWAY; break; // frintA
			// 0b001101 is unallocated
		case 0b001110: inst.op = ARM64_FRINTX; inst.frint.mode = FPR_CURRENT; break; // frintX
		case 0b001111: inst.op = ARM64_FRINT;  inst.frint.mode = FPR_CURRENT; break; // frintI
		case 0b010000: inst.op = ARM64_FRINT;  inst.frint.mode = FPR_ZERO;    inst.frint.bits = 32; break; // frint32Z
		case 0b010001: inst.op = ARM64_FRINTX; inst.frint.mode = FPR_CURRENT; inst.frint.bits = 32; break; // frint32X
		case 0b010010: inst.op = ARM64_FRINT;  inst.frint.mode = FPR_ZERO;    inst.frint.bits = 64; break; // frint64Z
		case 0b010011: inst.op = ARM64_FRINTX; inst.frint.mode = FPR_CURRENT; inst.frint.bits = 64; break; // frint64X
		default:
	...
	case Compare: {
		...
		switch (opcode) {
		case 0b00000: inst.op = ARM64_FCMP_REG;   break;
		case 0b01000: inst.op = ARM64_FCMP_ZERO;  break;
		case 0b10000: inst.op = ARM64_FCMPE_REG;  break;
		case 0b11000: inst.op = ARM64_FCMPE_ZERO; break;
		default:
			return errinst("scalar_floating_point/Compare: bad opcode");
		}

		inst.rn = regRn(binst);
		if (inst.op == ARM64_FCMP_REG || inst.op== ARM64_FCMPE_REG) {
			inst.rm = regRm(binst);
		}
		break;
	}
	...
	case CondCompare: {
		...
		inst.op = (signalling) ? ARM64_FCCMPE : ARM64_FCCMP;
	...
	case DataProc2: {
		...
		switch (opcode) {
		case 0b0000: inst.op = ARM64_FMUL;   break;
		case 0b0001: inst.op = ARM64_FDIV;   break;
		case 0b0010: inst.op = ARM64_FADD;   break;
		case 0b0011: inst.op = ARM64_FSUB;   break;
		case 0b0100: inst.op = ARM64_FMAX;   break;
		case 0b0101: inst.op = ARM64_FMIN;   break;
		case 0b0110: inst.op = ARM64_FMAXNM; break;
		case 0b0111: inst.op = ARM64_FMINNM; break;
		case 0b1000: inst.op = ARM64_FNMUL;  break;
		}
		inst.rd = regRd(binst);
		inst.rn = regRn(binst);
		inst.rm = regRm(binst);
...
static Inst decode_simd(u32 binst) {
	...
	case TableLookup: {
		...
		inst.op = (tbx) ? ARM64_TBX : ARM64_TBL;
		inst.imm = len + 1;
		inst.rd = regRd(binst);
	...
	case Permute: {
		...
		switch (opcode) {
		case 0b001: inst.op = ARM64_UZP1; break;
		case 0b010: inst.op = ARM64_TRN1; break;
		case 0b011: inst.op = ARM64_ZIP1; break;
		case 0b101: inst.op = ARM64_UZP2; break;
		case 0b110: inst.op = ARM64_TRN2; break;
		case 0b111: inst.op = ARM64_ZIP2; break;
		default:
	...
	case Copy: {
		...
		switch (imm4) {
		case 0b0000:
			inst.op = ARM64_DUP_ELEM;
			inst.flags |= (scalar) ? SIMD_SCALAR : 0;
			break;
		case 0b0001:
			inst.op = ARM64_DUP_GPR;
			inst.flags |= (prec != FSZ_D) ? W32 : 0;
			break;
		case 0b0011:
			inst.op = ARM64_INS_GPR;
			inst.flags = set_vec_arrangement(inst.flags, (prec << 1) | 1); // always 128-bit
			inst.flags |= (prec != FSZ_D) ? W32 : 0;
			break;
		case 0b0101: {
	...
	case ThreeSameExtra: {
		...
		switch (opcode) {
		case 0b0000: inst.op = ARM64_SQRDMLAH_VEC; inst.flags |= (scalar) ? SIMD_SCALAR : 0; break;
		case 0b0001: inst.op = ARM64_SQRDMLSH_VEC; inst.flags |= (scalar) ? SIMD_SCALAR : 0; break;
		case 0b0010: inst.op = ARM64_DOT_VEC;      inst.flags |= (U) ? 0 : SIMD_SIGNED;      break;
		case 0b1000:
		case 0b1001:
		case 0b1010:
		case 0b1011: // 10xx (xx = rot)
			inst.op = ARM64_FCMLA_VEC;
			switch (opcode & 0b11) {
	...
	case TwoRegMisc: {
		...
		switch (opcode) {
		case 0b00000: inst.op = (U) ? ARM64_REV32_VEC : ARM64_REV64_VEC; break;
		case 0b00001: inst.op = ARM64_REV16_VEC; break;
		case 0b00010: inst.op = ARM64_ADDLP; set_signedness = 1; break;
		case 0b00011: inst.op = (U) ? ARM64_USQADD : ARM64_SUQADD; set_scalarity = 1; break;
		case 0b00100: inst.op = (U) ? ARM64_CLZ_VEC : ARM64_CLS_VEC; break;
		case 0b00101:
			inst.op = (U) ? ((size==0) ? ARM64_NOT_VEC : ARM64_RBIT_VEC) : ARM64_CNT;
			inst.flags = set_vec_arrangement(inst.flags, (FSZ_B << 1) | Q);
			break;
		case 0b00110: inst.op = ARM64_ADALP; set_signedness = 1; break;
		case 0b00111: inst.op = (U) ? ARM64_SQNEG : ARM64_SQABS; inst.flags |= SIMD_SIGNED; set_scalarity = 1; break;
		case 0b01000: inst.op = (U) ? ARM64_CMGE_ZERO : ARM64_CMGT_ZERO; set_scalarity = 1; break;
		case 0b01001: inst.op = (U) ? ARM64_CMLE_ZERO : ARM64_CMEQ_ZERO; set_scalarity = 1; break;
		case 0b01010: inst.op = ARM64_CMLT_ZERO; set_scalarity = 1; break;
		case 0b01011: inst.op = (U) ? ARM64_NEG_VEC : ARM64_ABS_VEC; set_scalarity = 1; break;
		case 0b01100: is_fp = true; inst.op = (U) ? ARM64_FCMGE_ZERO : ARM64_FCMGT_ZERO; set_scalarity = 1; break;
		case 0b01101: is_fp = true; inst.op = (U) ? ARM64_FCMLE_ZERO : ARM64_FCMEQ_ZERO; set_scalarity = 1; break;
		case 0b01110: is_fp = true; inst.op = ARM64_FCMLT_ZERO; set_scalarity = 1; break;
		case 0b01111: is_fp = true; inst.op = (U) ? ARM64_FNEG_VEC : ARM64_FABS_VEC; set_scalarity = 1; break;
		case 0b10010: inst.op = (U) ? ARM64_SQXTUN : ARM64_XTN; set_scalarity = 1; break;
		case 0b10100: inst.op = ARM64_QXTN; set_signedness = 1; set_scalarity = 1; break;
		case 0b10110:
			inst.op = (U) ? ARM64_FCVTXN : ARM64_FCVTN;
			set_scalarity = 1;
		...
		case 0b11000:
			is_fp = true;
			inst.op = ARM64_FRINT_VEC;
			inst.frint.mode = (U) ? FPR_TIE_AWAY : ((size & 0b10) ? FPR_POS_INF : FPR_TIE_EVEN);
			break;
		case 0b11001:
			is_fp = true;
			if (U) {
				inst.op = (size & 0b10) ? ARM64_FRINT_VEC : ARM64_FRINTX_VEC;
			...
			break;
		case 0b11010: is_fp = true; inst.op = ARM64_FCVT_VEC; inst.fcvt.mode = (size & 0b10) ? FPR_POS_INF : FPR_TIE_EVEN; set_signedness = 1; set_scalarity = 1; break;
		case 0b11011: is_fp = true; inst.op = ARM64_FCVT_VEC; inst.fcvt.mode = (size & 0b10) ? FPR_ZERO : FPR_NEG_INF; set_signedness = 1; set_scalarity = 1; break;
		case 0b11100:
			is_fp = true; // URECPE, URSQRTE not actually FP, but still only use the size<0> bit to encode size
			if (size & 0b10) {
				inst.op = (U) ? ARM64_URSQRTE : ARM64_URECPE;
			} else {
				inst.op = ARM64_FCVT_VEC;
		...
		case 0b11101:
			is_fp = true;
			if (size & 0b10) {
				inst.op = (U) ? ARM64_FRSQRTE : ARM64_FRECPE;
			} else {
				inst.op = ARM64_CVTF_VEC;
				set_signedness = 1;
				set_scalarity = 1;
			}
			break;
		case 0b11110:
			is_fp = true;
			inst.op = (U) ? ARM64_FRINTX_VEC : ARM64_FRINT_VEC;
			inst.frint.bits = 32;
		...
		case 0b11111:
			is_fp = true;
			if (size & 0b10) {
				inst.op = (U) ? ARM64_FSQRT_VEC : ARM64_FRECPX;
				set_scalarity = 1;
			} else {
				inst.op = (U) ? ARM64_FRINTX_VEC : ARM64_FRINT_VEC;
				inst.frint.bits = 64;
		...
		if (set_signedness) {
			...
			if (inst.op == ARM64_FCVT_VEC || inst.op == ARM64_CVTF_VEC)
				inst.fcvt.sgn = !U;
		}
	...
	case Reduce: {
		...
		switch (opcode) {
		case 0b00011: inst.op = ARM64_ADDLV; set_signedness = 1; break;
		case 0b01010: inst.op = ARM64_MAXV;  set_signedness = 1; break;
		case 0b11010: inst.op = ARM64_MINV;  set_signedness = 1; break;
		case 0b11011: inst.op = (scalar) ? ARM64_ADDP : ARM64_ADDV; break;
		case 0b01100:
			if (scalar) {
				inst.op = (is_fmin) ? ARM64_FMINNMP : ARM64_FMAXNMP;
				if (U) {
				...
				inst.op = (is_fmin) ? ARM64_FMINNMV : ARM64_FMAXNMV;
		...
		case 0b01111:
			if (scalar) {
				inst.op = (is_fmin) ? ARM64_FMINP : ARM64_FMAXP;
				if (U) {
					va = (size&1) ? VA_2D : VA_2S;
				} else {
					va = VA_4H; // actually 2H (which does not exist)
				...
				inst.op = (is_fmin) ? ARM64_FMINV : ARM64_FMAXV;
				va = (((U) ? ((size&1) ? FSZ_D : FSZ_S) : FSZ_H) << 1) | Q;
			}
			break;
		}
	...
	case ThreeDiff: {
		...
		switch (opcode) {
		case 0b0000: inst.op = ARM64_ADDL; set_signedness = true; break;
		case 0b0001: inst.op = ARM64_ADDW; set_signedness = true; break;
		case 0b0010: inst.op = ARM64_SUBL; set_signedness = true; break;
		case 0b0011: inst.op = ARM64_SUBW; set_signedness = true; break;
		case 0b0100: inst.op = ARM64_ADDHN; inst.flags |= (U) ? SIMD_ROUND : 0; break;
		case 0b0101: inst.op = ARM64_ABAL; set_signedness = true; break;
		case 0b0110: inst.op = ARM64_SUBHN; inst.flags |= (U) ? SIMD_ROUND : 0; break;
		case 0b0111: inst.op = ARM64_ABDL; set_signedness = true; break;
		case 0b1000: inst.op = ARM64_MLAL_VEC; set_signedness = true; break;
		case 0b1001: inst.op = ARM64_SQDMLAL_VEC; inst.flags |= SIMD_SIGNED; set_scalarity = true; break;
		case 0b1010: inst.op = ARM64_MLSL_VEC; set_signedness = true; break;
		case 0b1011: inst.op = ARM64_SQDMLAL_VEC; inst.flags |= SIMD_SIGNED; set_scalarity = true; break;
		case 0b1100: inst.op = ARM64_MULL_VEC; set_signedness = true; break;
		case 0b1101: inst.op = ARM64_SQDMULL_VEC; inst.flags |= SIMD_SIGNED; set_scalarity = true; break;
		case 0b1110: inst.op = ARM64_PMULL; break;
		default:
	...
	case ThreeSame: {
		...
		switch (opcode) {
		case 0b00000: inst.op = ARM64_HADD; set_signedness = 1; break;
		case 0b00001: inst.op = ARM64_QADD; set_signedness = 1; set_scalarity = 1; break;
		case 0b00010: inst.op = ARM64_HADD; set_signedness = 1; inst.flags |= SIMD_ROUND; break;
		case 0b00011:
			// size acts as sub-opcode
			switch (size) {
			case 0b00: inst.op = (U) ? ARM64_EOR_VEC : ARM64_AND_VEC;     break;
			case 0b01: inst.op = (U) ? ARM64_BSL     : ARM64_BIC_VEC_REG; break;
			case 0b10: inst.op = (U) ? ARM64_BIT     : ARM64_ORR_VEC_REG; break;
			case 0b11: inst.op = (U) ? ARM64_BIF     : ARM64_ORN_VEC;     break;
			}
			inst.flags = set_vec_arrangement(inst.flags, (FSZ_B << 1) | Q);
			break;
		case 0b00100: inst.op = ARM64_HSUB; set_signedness = 1; break;
		case 0b00101: inst.op = ARM64_QSUB; set_signedness = 1; set_scalarity = 1; break;
		case 0b00110: inst.op = (U) ? ARM64_CMHI_REG : ARM64_CMGT_REG; set_scalarity = 1; break;
 		case 0b00111: inst.op = (U) ? ARM64_CMHS_REG : ARM64_CMGE_REG; set_scalarity = 1; break;
		case 0b01000: inst.op = ARM64_SHL_REG; set_signedness = 1; set_scalarity = 1; break;
		case 0b01001: inst.op = ARM64_QSHL_REG; set_signedness = 1; set_scalarity = 1; break;
		case 0b01010: inst.op = ARM64_SHL_REG; set_signedness = 1; set_scalarity = 1; inst.flags |= SIMD_ROUND; break;
		case 0b01011: inst.op = ARM64_QSHL_REG; set_signedness = 1; set_scalarity = 1; inst.flags |= SIMD_ROUND; break;
		case 0b01100: inst.op = ARM64_MAX_VEC; set_signedness = 1; break;
		case 0b01101: inst.op = ARM64_MIN_VEC; set_signedness = 1; break;
		case 0b01110: inst.op = ARM64_ABD; set_signedness = 1; break;
		case 0b01111: inst.op = ARM64_ABA; set_signedness = 1; break;
		case 0b10000: inst.op = (U) ? ARM64_SUB_VEC : ARM64_ADD_VEC; set_scalarity = 1; break;
		case 0b10001: inst.op = (U) ? ARM64_CMEQ_REG : ARM64_CMTST; set_scalarity = 1; break;
		case 0b10010: inst.op = (U) ? ARM64_MLS_VEC : ARM64_MLA_VEC; break;
		case 0b10011: inst.op = (U) ? ARM64_PMUL : ARM64_MUL_VEC; break;
		case 0b10100: inst.op = ARM64_MAXP; set_signedness = 1; break;
		case 0b10101: inst.op = ARM64_MINP; set_signedness = 1; break;
		case 0b10110: inst.op = ARM64_SQDMULH_VEC; set_scalarity = 1; inst.flags |= (U) ? SIMD_ROUND : 0; break;
		case 0b10111: inst.op = ARM64_ADDP_VEC; break;
		case 0b11000: is_fp = true; inst.op = (U) ? ((size&0b10) ? ARM64_FMINNMP_VEC : ARM64_FMAXNMP_VEC) : ((size&0b10) ? ARM64_FMINNM_VEC : ARM64_FMAXNM_VEC); break;
		case 0b11001: is_fp = true; inst.op = (U) ? ((size&0b10) ? ARM64_FMLSL2_VEC : ARM64_FMLAL2_VEC) : ((size&0b10) ? ARM64_FMLS_VEC : ARM64_FMLA_VEC); break;
		case 0b11010:
			is_fp = true;
			inst.op = (U) ? ((size&0b10) ? ARM64_FABD_VEC : ARM64_FADDP_VEC) : ((size&0b10) ? ARM64_FSUB_VEC : ARM64_FADD_VEC);
			if (inst.op == ARM64_FABD_VEC) {
				set_scalarity = 1;
			}
			break;
		case 0b11011: is_fp = true; inst.op = (U) ? ARM64_FMUL_VEC : ((scalar) ? ARM64_FMULX : ARM64_FMULX_VEC); break;
		case 0b11100: is_fp = true; inst.op = (U) ? ((size&0b10) ? ARM64_FCMGT_REG : ARM64_FCMGE_REG) : ARM64_FCMEQ_REG; set_scalarity = 1; break;
		case 0b11101: is_fp = true; inst.op = (U) ? ((size&0b10) ? ARM64_FACGT : ARM64_FACGE) : ((size&0b10) ? ARM64_FMLSL_VEC : ARM64_FMLAL_VEC); break; set_scalarity = 1; break;
		case 0b11110: is_fp = true; inst.op = (U) ? ((size&0b10) ? ARM64_FMINP_VEC : ARM64_FMAXP_VEC) : ((size&0b10) ? ARM64_FMIN_VEC : ARM64_FMAX_VEC); break;
		case 0b11111: is_fp = true; inst.op = (U) ? ARM64_FDIV_VEC : ((size&0b10) ? ARM64_FRSQRTS_VEC : ARM64_FRECPS_VEC); break;
		}
	...
	case ModifiedImm: {
		...
		case 0b0111: // 0xx1 → ORR(32), BIC(32)
			inst.op = (op) ? ARM64_BIC_VEC_IMM : ARM64_ORR_VEC_IMM;
			inst.flags = set_vec_arrangement(inst.flags, (FSZ_S << 1) | Q);
			inst.imm = imm8 << zlsl;
			break;
		case 0b1001:
	...
	case ShiftByImm: {
		...
		switch (opcode) {
		case 0b00000: inst.op = ARM64_SHR; break;
		case 0b00010: inst.op = ARM64_SRA; break;
		case 0b00100: inst.op = ARM64_SHR; inst.flags |= SIMD_ROUND; break;
		case 0b00110: inst.op = ARM64_SRA; inst.flags |= SIMD_ROUND; break;
		case 0b01000: inst.op = ARM64_SRI; break;
		case 0b01010: inst.op = (U) ? ARM64_SLI : ARM64_SHL_IMM; left_shift = 1; set_signedness = 0; break;
		case 0b01110: inst.op = ARM64_QSHL_IMM; left_shift = 1; break;
		case 0b10000:
		case 0b10001: // 1000x (x = round?)
			inst.op = (U) ? ARM64_SQSHRUN : ARM64_SHRN;
			set_signedness = 0;
			set_scalarity = 0;
			inst.flags |= (opcode & 1) ? SIMD_ROUND : 0;
			break;
		case 0b10010: inst.op = ARM64_QSHRN; break;
		case 0b10011: inst.op = ARM64_QSHRN; inst.flags |= SIMD_ROUND; break;
		case 0b10100: inst.op = ARM64_SHLL; left_shift = 1; set_scalarity = 0; break;
		case 0b11100: inst.op = ARM64_CVTF_VEC; break; // fixed-point
		case 0b11111: inst.op = ARM64_FCVT_VEC; break; // fixed-point
		default:
		...
		if (inst.op == ARM64_CVTF_VEC || inst.op == ARM64_FCVT_VEC) {
			inst.fcvt.mode = FPR_ZERO;
			inst.fcvt.fbits = imm;
			inst.fcvt.sgn = !U;
		} else {
	...
	case IndexedElem: {
		...
		case 0b0000:
			if (U) {
				inst.op = ARM64_MLA_ELEM;
				break;
			}

			inst.op = ARM64_FMLAL_ELEM;
			inst.flags = set_vec_arrangement(inst.flags, FSZ_H << 1 | Q);
			inst.imm = hlm;
			short_rm = true; // M is used in index
			break;
		case 0b0001: inst.op = ARM64_FMLA_ELEM; set_scalarity  = true; break;
		case 0b0010: inst.op = ARM64_MLAL_ELEM; set_signedness = true; break;
		case 0b0011: inst.op = ARM64_SQDMLAL_ELEM; set_scalarity = true; inst.flags |= SIMD_SIGNED; break;
		case 0b0100:
			if (U) {
				inst.op = ARM64_MLS_ELEM;
				break;
			}

			inst.op = ARM64_FMLSL_ELEM;
			inst.flags = set_vec_arrangement(inst.flags, FSZ_H << 1 | Q);
			inst.imm = hlm;
			short_rm = true; // M is used in index
			break;
		case 0b0101: inst.op = ARM64_FMLS_ELEM; set_scalarity  = true; break;
		case 0b0110: inst.op = ARM64_MLSL_ELEM; set_signedness = true; break;
		case 0b0111: inst.op = ARM64_SQDMLSL_ELEM; set_scalarity = true; inst.flags |= SIMD_SIGNED; break;
		case 0b1000:
			if (!U) {
				inst.op = ARM64_MUL_ELEM;
				break;
			}

			inst.op = ARM64_FMLAL2_ELEM;
			inst.flags = set_vec_arrangement(inst.flags, FSZ_H << 1 | Q);
			inst.imm = hlm;
			short_rm = true; // M is used in index
			break;
		case 0b1001: inst.op = (U) ? ARM64_FMULX_ELEM : ARM64_FMUL_ELEM; set_scalarity = true; break;
		case 0b1010: inst.op = ARM64_MULL_ELEM; set_signedness = true; break;
		case 0b1011: inst.op = ARM64_SQDMULL_ELEM; set_scalarity = true; inst.flags |= SIMD_SIGNED; break;
		case 0b1100:
			if (!U) {
				inst.op = ARM64_SQDMULH_ELEM;
				set_scalarity = true;
				inst.flags |= SIMD_SIGNED;
				break;
			}
			inst.op = ARM64_FMLSL2_ELEM;
			inst.flags = set_vec_arrangement(inst.flags, FSZ_H << 1 | Q);
			inst.imm = hlm;
			short_rm = true; // M is used in index
			break;
		case 0b1101:
			inst.op = (U) ? ARM64_SQRDMLAH_ELEM : ARM64_SQDMULH_ELEM; // SQRDMULH, SQRDMLAH
			inst.flags |= SIMD_SIGNED | SIMD_ROUND;
			set_scalarity = true;
			break;
		case 0b1110: // e.g.: udot v0.2s, v1.8b, v2.4b[idx] (idx = 0..3)
			inst.op = ARM64_DOT_ELEM;
			inst.flags = set_vec_arrangement(inst.flags, FSZ_B << 1 | Q);
			inst.imm = hlm >> 1; // H:L
			set_signedness = true;
			break;
		case 0b1111: inst.op = ARM64_SQRDMLSH_ELEM; inst.flags |= SIMD_SIGNED | SIMD_ROUND; set_scalarity = true; break;
		}
...
//
// found in the ARM ARM64 ISA manual, specialised to bits(64). The imm8
...
/// @brief 从输入缓冲区解码n个二进制指令，并且将它们放在输出缓冲区中，该缓冲区必须有n个Insts的空间。
/// @param in 二进制指令数据
/// @param n 二进制指令个数
/// @param out 输出指令缓冲区
/// @return 指令个数
...
void print_inst(Inst inst) {
	...
	case ARM64_UNKNOWN: {
		...
		// 打印未知指令为小端十六进制字节。这种格式可以被在线反汇编器（ODA）使用。
		printf("??? %02x %02x %02x %02x\n", (binst>>0) & 0xff, (binst>>8) & 0xff,
			(binst>>16) & 0xff, (binst>>24) & 0xff);
		return;
	}
	case ARM64_ERROR:   printf("error \"%s\"\n", inst.error); return;
	case ARM64_UDF:     printf("udf\n");                      return;
	default:
		break; // normal instruction
	}

	// 在这里不消除歧义——所有说明都已打印出来相同；例如，带有两个立即数的指令有也打印了imm字段。
	printf("%-12s %c %c%d, %c%d, %c%d, imm=%lu, offset=%+ld, fimm=%f, imm2=(%u,%u), flags=0b %o%o%o %o%o%o %o%o\n",
		arm64_insts[inst.op], flagsch,
		regch, inst.rd, regch, inst.rn, regch, inst.rm,
		(unsigned long)inst.imm, (long)inst.offset, inst.fimm, inst.bfm.lsb, inst.bfm.width,
