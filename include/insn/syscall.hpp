...
#ifndef INSN_SYSCALL_HPP
#define INSN_SYSCALL_HPP
...
/// 获取寄存器值
#define GET(reg, name) u64 name = vm.get_gp_reg(reg);

/// syscall函数指针
typedef u64 (* syscall_t)(VM &vm);

/// @brief 未实现方法
/// @param m 虚拟机对象
/// @return 
static u64 sys_unimplemented(VM &vm) {
    fatalf("unimplemented syscall: %llu", vm.get_gp_reg(a7));
}
 ...
 * malloc/free
 */
static u64 sys_brk(VM &vm) {
    GET(a0, addr);
    if (addr == 0) addr = vm.mmu.alloc;
    assert(addr >= vm.mmu.base);
    i64 incr = (i64)addr - vm.mmu.alloc;
    vm.mmu.MMU_alloc(incr);
    return addr;
}
