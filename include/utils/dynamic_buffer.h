/**
 * @file include/utils/dynamic_buffer.h
...
#include "utils/byte_value_storage.h"
...
public:
	...
	void setEndianness(utils::Endianness endianness);
	utils::Endianness getEndianness() const;
	...
	template <typename T> T read(
			uint32_t pos,
			utils::Endianness endianness
					= utils::Endianness::UNKNOWN) const
	{
		static_assert(
				std::is_integral<T>::value,
				"utils::DynamicBuffer::read can only accept integral types"
		);

		// In case of non-specified endianness, use the default one assigned
		// to DynamicBuffer itself
		if (endianness == utils::Endianness::UNKNOWN)
	...
	template <typename T> void write(
			...
			uint32_t pos,
			utils::Endianness endianness
					= utils::Endianness::UNKNOWN)
	{
		static_assert(
				std::is_integral<T>::value,
				"utils::DynamicBuffer::write can only accept integral types"
		);
...
private:
			...
			utils::Endianness endianness)
	{
		...
		for (uint32_t i = 0; i < bytesToWrite; ++i)
		{
			switch (endianness)
			{
				case utils::Endianness::LITTLE:
					_data[pos + i] = (data >> (i << 3)) & 0xFF;
					break;
				case utils::Endianness::BIG:
	...
	template <typename T> T readImpl(
			uint32_t pos,
			utils::Endianness endianness) const
	{
